...
class MilestoneFactory:
    ...
    @staticmethod
    def create_business_startup(
        trigger_year: int,
        startup_cost: float,
        monthly_operating_cost: float,
        expected_monthly_revenue: float,
        business_loan_amount: float = 0,
        loan_interest_rate: float = 0.06,
        loan_term_years: int = 5,
        equipment_cost: float = 0,
        equipment_depreciation_rate: float = 0.2,
        tax_deduction_rate: float = 0.3,
        revenue_growth_rate: float = 0.1
    ) -> Milestone:
        milestone = Milestone("Business Startup", trigger_year, "Business")

        # Add initial startup cost as one-time expense
        milestone.add_one_time_expense(startup_cost)

        # Add business equipment as depreciating asset if applicable
        if equipment_cost > 0:
            milestone.add_asset(DepreciableAsset("Business Equipment", equipment_cost, equipment_depreciation_rate))

        # Add business loan if applicable
        if business_loan_amount > 0:
            business_loan = Loan("Business Loan", business_loan_amount, loan_interest_rate, loan_term_years)
            milestone.add_liability(business_loan)

        # Add monthly operating expenses
        milestone.add_recurring_expense(VariableExpense("Business Operating Costs", monthly_operating_cost * 12))

        # Create a custom Income class for business revenue with growth
        class BusinessIncome(Income):
            def __init__(self, annual_amount: float, growth_rate: float, start_year: int):
                super().__init__("Business Revenue", annual_amount, growth_rate, start_year)

            def calculate_income(self, year: int) -> float:
                if year < self.start_year:
                    return 0
                # Apply exponential growth to revenue
                years_active = year - self.start_year
                return self.annual_amount * (1 + self.growth_rate) ** years_active

        # Add business revenue as income
        business_income = BusinessIncome(
            expected_monthly_revenue * 12,  # Convert to annual
            revenue_growth_rate,
            trigger_year
        )
        milestone.add_income_adjustment(business_income)

        # Add tax deductions as negative expenses (savings)
        class BusinessTaxDeduction(FixedExpense):
            def __init__(self, operating_cost: float, deduction_rate: float):
                super().__init__("Business Tax Deductions", -operating_cost * deduction_rate * 12)

        tax_deduction = BusinessTaxDeduction(monthly_operating_cost, tax_deduction_rate)
        milestone.add_recurring_expense(tax_deduction)
